...
var modal = document.getElementById('flappyModal');

// Get the button that opens the modal
var btn = document.getElementById("flappyModalButton");
...
function updateCanvas() {
    ...
    d2.clear();
    d2.frameNo += 1;
    if (d2.frameNo == 1 || everyinterval(150)) {
        x = d2.canvas.width;
        minHeight = 20;
        maxHeight = 75;
        ...
        myObstacles.push(new canvasImageObject(50, height, "media/stick50_100.png", x, 0, 0));
        myObstacles.push(new canvasImageObject(50, x - height - gap, "media/stick50_100.png", x, height + gap, 0));
    }
    for (i = 0; i < myObstacles.length; i += 1) {
        myObstacles[i].x += -1;
...
function startDemo() {                                                      // All objects need to be instantiated here 
    background = new canvasBackground(800, 200, "media/background.png", 0, 0);     // before the "start" function is called.
    myScore = new canvasImageObject("30px", "Consolas", "black", 280, 40, "text");
    player = new canvasImageObject(75, 40, "media/duck.png", 10, 100,0);
    player.gravity = 0.10;
    d2.start();
}
